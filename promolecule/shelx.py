    ...
    for line_number, line in enumerate(contents, start=1):
        try:
            ...
            if key == "END":
                ...
                shelx_dict["ATOM"].append(parse_atom_line(shelx_dict["SFAC"], line))
            else:
...
def atom_lines(atoms):
    return "\n".join(atoms)


def to_res_contents(data):
    SHELX_FORMATTERS = {
    ...
    for key in SHELX_FORMATTERS:
        if data[key] != []:
            sections.append(SHELX_FORMATTERS[key](data[key]))
    return "\n".join(sections)
