...
class PowderPattern:
    ...
    def __init__(self, two_theta, f2, **kwargs):
        ...
        self.source = kwargs.get("source", "unknown" if self.wavelength != LAMBDA_Cu else "Cu")
        self.bins = kwargs.get("bins", 1000)
        self.bin_edges, self.bin_heights = np.histogram(
            self.two_theta, bins=self.bins,
            weights=self.f2
        )
    ...
    def plot(self, ax=None, **kwargs):
        if ax is None:
            ax = plt.gca()
        ax.hist(self.two_theta, bins=self.bins, weights=self.f2)
        ax.set_xlabel(kwargs.get("xlabel", r"2$\theta$"))
        ax.set_ylabel(kwargs.get("ylabel", r"Intensity"))
        ax.set_title(kwargs.get("title", f"Powder pattern in range {self.two_theta_range}"))
        return ax
