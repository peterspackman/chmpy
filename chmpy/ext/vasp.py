...
def poscar_string(crystal, name="vasp_input"):
    ...
    direct = "\n".join(f"{x:12.8f} {y:12.8f} {z:12.8f}" for x, y, z in crystal.unit_cell.direct)
...
def generate_vasp_inputs(crystal, dest="."):
    ...
    if not dest.exists():
        dest.mkdir()
    Path(dest, "POSCAR").write_text(
        poscar_string(crystal, name=dest.name)
    )
    Path(dest, "INCAR").write_text(
        incar_string()
    )
    Path(dest, "KPOINTS").write_text(
        kpoints_string()
    )
