...
def _is_symmorphic(symmetry_operations, denom=12) -> bool:
    """Return True if a common origin makes every operation translation-free.

    A space group is symmorphic iff there is a single origin shift p such that
    every operation (R, t) becomes (R, c) with c a centering (lattice)
    translation. This is a *common*-origin condition: a per-rotation test would
    wrongly accept e.g. I2_12_12_1 (#24), whose screw axes share no common
    point. Searches origin shifts p = k/denom on an integer grid using exact
    arithmetic in units of 1/denom; denom=12 reproduces the canonical 73
    symmorphic space groups.
    """
    rotations = np.array(
        [np.round(o.rotation).astype(int) for o in symmetry_operations]
    )
    translations = np.array(
        [
            np.round(np.asarray(o.translation) * denom).astype(int) % denom
            for o in symmetry_operations
        ]
    )
    identity = np.eye(3, dtype=int)
    rot_minus_identity = rotations - identity
    centering = {
        tuple(translations[i])
        for i in range(len(symmetry_operations))
        if np.array_equal(rotations[i], identity)
    }
    for shift in product(range(denom), repeat=3):
        shifted = (translations + rot_minus_identity @ np.array(shift)) % denom
        if all(tuple(row) in centering for row in shifted):
            return True
    return False
...
class SpaceGroup:
    ...
    def is_symmorphic(self) -> bool:
        """
        Whether this space group is symmorphic.

        A space group is symmorphic if it can be generated by its point group
        (acting through a common origin) together with the lattice translations,
        i.e. it contains no intrinsic screw axes or glide planes. There are 73
        symmorphic space groups among the 230.

        Examples:
            >>> SpaceGroup(225).is_symmorphic  # Fm-3m
            True
            >>> SpaceGroup(14).is_symmorphic  # P2_1/c
            False
            >>> SpaceGroup(23).is_symmorphic  # I222
            True
            >>> SpaceGroup(24).is_symmorphic  # I2_12_12_1
            False
        """
        return _is_symmorphic(self.symmetry_operations)

    def is_systematically_absent(self, hkl):
        """
        Whether reflections are systematically absent (forbidden by symmetry).

        A reflection h is systematically absent if some symmetry operation
        (R, t) leaves it invariant (h R = h) but introduces a non-integral
        phase (h . t is not an integer), forcing the structure factor to zero
        regardless of the atomic content. This single rule reproduces all of the
        integral (centering), zonal (glide-plane) and serial (screw-axis)
        reflection conditions of the International Tables.

        Args:
            hkl: a single (3,) reflection index or an (N, 3) array of indices

        Returns:
            bool for a single reflection, or an (N,) boolean array, where True
            means the reflection is systematically absent.

        Examples:
            >>> sg = SpaceGroup(14)  # P2_1/c: 2_1 || b, c-glide perp b
            >>> sg.is_systematically_absent((0, 1, 0))  # 0k0, k odd
            True
            >>> sg.is_systematically_absent((1, 0, 1))  # h0l, l odd
            True
            >>> sg.is_systematically_absent([[0, 2, 0], [1, 0, 2]]).tolist()
            [False, False]
        """
        hkl_array = np.atleast_2d(np.asarray(hkl, dtype=int))
        absent = np.zeros(len(hkl_array), dtype=bool)
        for op in self.symmetry_operations:
            rotation = np.round(op.rotation).astype(int)
            # exact phase in twelfths (translations are multiples of 1/12)
            translation = np.round(op.translation * 12).astype(int)
            # reflections invariant under the rotation part: h R = h
            invariant = np.all(hkl_array @ rotation == hkl_array, axis=1)
            if not invariant.any():
                continue
            phase = hkl_array @ translation
            absent |= invariant & (phase % 12 != 0)
        if np.ndim(hkl) == 1:
            return bool(absent[0])
        return absent

    def reflection_conditions(self):
        """
        The reflection conditions (systematic absences) for this space group,
        grouped by reflection class as in the International Tables.

        Returns:
            list[ReflectionCondition]: one entry per reflection class that
            carries a condition, ordered general -> zonal -> axial. An empty
            list means there are no conditions (e.g. P1, P-1).

        Examples:
            >>> for c in SpaceGroup(14).reflection_conditions():  # P2_1/c
            ...     print(c)
            h0l: l=2n
            0k0: k=2n
        """
        from .reflection_conditions import reflection_conditions

        return reflection_conditions(self.symmetry_operations)
